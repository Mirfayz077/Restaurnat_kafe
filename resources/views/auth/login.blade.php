    ...
    <div class="grid w-full gap-6 lg:grid-cols-[1.08fr_0.92fr]">
        <section class="soft-panel rounded-[2rem] border border-white/10 p-8 lg:p-10">
            <div class="flex flex-wrap items-center gap-3">
                ...
                <a href="{{ route('home') }}" class="rounded-full border border-white/10 bg-slate-950/40 px-4 py-2 text-xs uppercase tracking-[0.25em] text-slate-300 transition hover:text-white">
                    Back to main page
                </a>
            </div>
        ...
        <section class="soft-panel rounded-[2rem] border border-white/10 p-8">
            <p class="text-xs uppercase tracking-[0.35em] text-amber-200">Secure access</p>
            <h2 class="mt-3 text-3xl font-semibold text-white">Accountga kiring</h2>
            ...
            <form id="loginForm" action="{{ route('login.store') }}" method="POST" class="mt-8 space-y-5">
                ...
                <label class="block">
                    <span class="mb-2 block text-sm text-slate-300">Parol</span>
                    <div class="relative">
                        <input
                            id="password"
                            type="password"
                        ...
                        <button
                            id="togglePassword"
                            type="button"
                            class="absolute inset-y-0 right-0 px-4 text-sm text-slate-400 transition hover:text-white"
                        >
                            Show
                        </button>
                    </div>
                ...
                <button type="submit" class="btn btn-warning w-full rounded-2xl text-base">
                    Kirish
                </button>
            </form>
        </section>
